...
console.log('CLICKUP_TOKEN exists:', !!CLICKUP_TOKEN);
...
app.post('/api/submit-brief', async (req, res) => {
  try {
    ...
    if (!CLICKUP_TOKEN) {
      ...
      return res.status(500).json({ success: false, message: 'Server configuration error: Missing ClickUp token' });
    }
    if (!CLICKUP_LIST_ID) {
      console.error('CLICKUP_LIST_ID is not set');
      return res.status(500).json({ success: false, message: 'Server configuration error: Missing ClickUp list ID' });
    }
    ...
    console.error('Error creating ClickUp task:', error.response ? error.response.data : error.message);
    res.status(500).json({
      success: false,
      message: 'Failed to submit brief. ' + (error.response?.data?.err || error.message),
      error: error.message
    });
  }
});
